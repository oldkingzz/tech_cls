"""
Case Western数据集类，继承自基类dataset_yoto
"""
...
class Case_Dataset(dataset_yoto):
    """Case Western数据集类，继承自基类dataset_yoto"""
    ...
    def _load_samples(self, data_type):
        ...
        # 遍历数据目录中的故障类型目录
        fault_type_dirs = [d for d in os.listdir(self.data_dir) if os.path.isdir(os.path.join(self.data_dir, d))]
        
        for fault_dir in fault_type_dirs:
            fault_path = os.path.join(self.data_dir, fault_dir)
            fault_type = fault_dir  # 文件夹名称就是故障类型(inner, outer, ball, normal)
            
            # 获取所有CSV文件
            csv_files = [f for f in os.listdir(fault_path) if f.endswith('.csv')]
            
            # 处理所有文件
            for csv_file in csv_files:
                try:
                    file_path = os.path.join(fault_path, csv_file)
                    ...
                    # 提取元数据
                    fault_size = df['故障尺寸(inch)'].iloc[0] if '故障尺寸(inch)' in df.columns else 0.0
                    rpm = df['RPM'].iloc[0] if 'RPM' in df.columns else 0.0
                        ...
                        if data_type == 0:  # 故障分类
                            ...
                            samples.append({
                                ...
                                'metadata': {
                                    ...
                                    'file': csv_file,
                                    'segment': i,
                                    'subdir': os.path.basename(fault_path)
                                }
                            })
                        
                        elif data_type == 1:  # 寿命预测
                            print("警告: Case Western数据集不支持寿命预测任务")
                        
                except Exception as e:
                    print(f"加载文件 {csv_file} 出错: {e}")
        
        print(f"Case Western数据集共加载了 {len(samples)} 个样本")
        return samples
...
if __name__ == "__main__":
    ...
    if len(dataset) > 0:
        ...
        plt.xlabel("采样点")
        plt.ylabel("振幅")
        plt.savefig("case_sample.png")
        print("已保存波形图: case_sample.png") 