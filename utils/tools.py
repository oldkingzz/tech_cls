...
def get_cosine_schedule_with_warmup(optimizer, warmup_steps, training_steps):
    """
    创建一个带有预热的余弦学习率调度器
...
def cleanup_temp_files():
    ...
    清理/tmp目录下的临时文件，包括以'pymp-'或'tmp'开头的文件和目录
    """
    # 清理/tmp目录下的临时文件
    tmp_dir = '/tmp'
    for filename in os.listdir(tmp_dir):
        if filename.startswith('pymp-') or filename.startswith('tmp'):
            try:
...
def load_content(args):
    if 'ETT' in args.data:
        ...
        file = args.data
    with open('/root/Time-LLM-main/dataset/prompt_bank/{0}.txt'.format(file), 'r') as f:
        content = f.read()
    return content

def vali_c(args, accelerator, model, vali_loader, criterion):
    total_loss = []
    total_correct = 0
    total_samples = 0
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y) in tqdm(enumerate(vali_loader)):
            # ��������ת��
            batch_x = batch_x.float().to(accelerator.device)
            batch_y = batch_y.long().to(accelerator.device)  # ����������Ҫ long ���ͱ�ǩ

            # ǰ�򴫲�
            outputs = model(batch_x)  # ����ģ��ֱ����� logits
            
            # ������ʧ
            loss = criterion(outputs, batch_y)  # ��������ʧ
